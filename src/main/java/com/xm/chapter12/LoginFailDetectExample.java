...
public class LoginFailDetectExample {
    public static void main(String[] args) throws Exception {
        ...
        // 2. 定义模式 连续三次登陆失败
        Pattern<LoginEvent, LoginEvent> pattern = Pattern.<LoginEvent>begin("first") // 第一次登陆失败事件
                ...
                .next("third") // 紧跟着第三次登陆失败事件
                .where(new SimpleCondition<LoginEvent>() {
                    @Override
                    public boolean filter(LoginEvent value) throws Exception {
                        return value.eventType.equals("fail");
                    }
                }).within(Time.seconds(10)); // 设置10s后就抛弃此次匹配
        ...
        SingleOutputStreamOperator<String> warningStream = patternStream.select(new PatternSelectFunction<LoginEvent, String>() {
            ...
            public String select(Map<String, List<LoginEvent>> map) throws Exception {
                ...
                return firstFailEvent.userId + "连续三次登陆失败！登陆时间: " +
