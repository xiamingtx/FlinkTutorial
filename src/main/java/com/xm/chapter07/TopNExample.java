...
import com.xm.chapter06.UrlCountViewExample;
import com.xm.chapter06.UrlViewCount;
...
public class TopNExample {
    public static void main(String[] args) throws Exception {
        ...
        // 2. 对于同一窗口统计出的访问量 进行收集和排序 这样可以实现并行计算
    ...
    public static class TopNProcessResult extends KeyedProcessFunction<Long, UrlViewCount, String> {
        ...
        public void onTimer(long timestamp, KeyedProcessFunction<Long, UrlViewCount, String>.OnTimerContext ctx, Collector<String> out) throws Exception {
            ...
            // 取list前两个 包装信息输出
            for (int i = 0; i < n; i ++ ) {
                UrlViewCount curTuple = urlViewCountArrayList.get(i);
                String info = "No. " + (i + 1) + " " + " url: " + curTuple.url + " 访问量: " + curTuple.count + "\n";
                result.append(info);
            }
            result.append("-----------------------------------\n");
